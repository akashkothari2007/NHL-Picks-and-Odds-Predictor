...
def getSingleGameFeatureSet(home_team_ids, away_team_ids, game_date=None):
    """
    ...
    print(f"Calculating features for {len(home_team_ids)} games...")
    
    all_features = []
    
    for home_id, away_id in zip(home_team_ids, away_team_ids):
        features = calculate_single_game_features(home_id, away_id, game_date, past_games)
...
def calculate_single_game_features(home_id, away_id, game_date, past_games):
    """
    Calculate features for ONE game.
    
    Args:
        home_id: Home team ID
        away_id: Away team ID
        game_date: Game date
        past_games: DataFrame of all past games
    ...
    if len(home_games) < 5:
        print(f"⚠️  Warning: Only {len(home_games)} games for home team {home_id}")
        return get_neutral_features()
    
    # Last 5 games for rolling stats
    ...
    # Win percentages from most recent game's cumulative stats
    home_last = home_games.iloc[-1]
    home_home_wins = home_last['W_HOME']
    home_home_losses = home_last['L_HOME']
    home_away_wins = home_last['W_ROAD']
    home_away_losses = home_last['L_ROAD']
    ...
    home_total_wins = home_last['W']
    home_total_games = home_total_wins + home_last['L']
    ...
    if len(away_games) < 5:
        print(f"⚠️  Warning: Only {len(away_games)} games for away team {away_id}")
        return get_neutral_features()
    ...
    away_home_wins = away_last['W_HOME']
    away_home_losses = away_last['L_HOME']
    away_away_wins = away_last['W_ROAD']
    away_away_losses = away_last['L_ROAD']
    ...
    away_total_wins = away_last['W']
    away_total_games = away_total_wins + away_last['L']
    ...
    # === HEAD-TO-HEAD ===
    # Get games where EITHER team played
    h2h_games = past_games[
        (past_games['TEAM_ID'] == home_id) | (past_games['TEAM_ID'] == away_id)
    ]
    
    # Find common game IDs (where they played EACH OTHER)
    ...
    if len(common_game_ids) >= 2:
        ...
        # Home team's record in these matchups
